...
data_D %>%
  ggplot(aes(x = perc_firing, y = num_bins, fill = species)) +
    geom_point(aes(shape = species), position = 'jitter') +
    geom_boxplot() +
    REPLACE_ME(values = c(col_hb, col_zb)) +
    REPLACE_ME(values = c(24, 21))

# De-emphasize boxplots, remove outliers
data_D %>%
  ggplot(aes(x = perc_firing, y = num_bins, fill = species)) +
    geom_boxplot(REPLACE_ME, outlier.size = 0) +
    geom_point(aes(shape = species), position = 'jitter') +
...
read_csv("./gaedeetal_data/Fig3E_data.csv") %>%
  REPLACE_ME(species = c("hb", "zb", "pg"))

# Pull the y-values down from the header
read_csv("./gaedeetal_data/Fig3E_data.csv") %>%
  mutate(species = c("hb", "zb", "pg")) %>%
  gather(key = REPLACE_ME, value = REPLACE_ME, REPLACE_ME)
...
data_E %>%
  ggplot(aes(x = speed, y = prop, fill = species)) +
    geom_col(REPLACE_ME = 'dodge', REPLACE_ME)

# Space out column groups, thin out the columns, thin the outline
data_E %>%
  ggplot(aes(x = speed, y = prop, fill = species)) +
    geom_col(
      position = REPLACE_ME(width = 0.75), # Dodge width
      REPLACE_ME = 0.75,                   # Column width
      REPLACE_ME = 0.2,                    # Outline thickness
      colour = "black"
    )
...
data_E %>%
  ggplot(aes(x = speed, y = prop, fill = species)) +
    ...
    REPLACE_ME(values = c(col_hb, col_zb, col_pg)) +
    REPLACE_ME(
...
read_csv("gaedeetal_data/Fig3B_data.csv") %>%
  REPLACE_ME(key = REPLACE_ME, value = REPLACE_ME, REPLACE_ME)
...
# Plot the data
data_B %>%
  ggplot(aes(REPLACE_ME)) +
    geom_point(aes(REPLACE_ME)) +
    geom_line(aes(REPLACE_ME))
...
data_B %>%
  ggplot(aes(x = speed, y = firing_rate)) +
    ...
    geom_point(aes(colour = direction, shape = direction, REPLACE_ME)) +
    annotate(
      geom = 'segment',
      x = 1.3, xend = 1.78,
      y = 1.2, yend = 1.2,
...
# Calculate mean of proporions by species and speed
read_csv("./gaedeetal_data/Fig3F_data.csv") %>%
  filter(!is.na(dir1.area))  %>%
  select(starts_with("sp")) %>%
  rename_all(~c("species",as.character(speeds_F))) %>%
  ...
  gather(key = speed, value = prop, -species) %>%
  REPLACE_ME(species, speed) %>%
  REPLACE_ME(prop = mean(prop)) %>%
  REPLACE_ME
